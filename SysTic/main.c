...
unsigned long AS;     //Atrial sensor "input -> PF4"

 

//   Function Prototypes
void PortF_Init(void);
//void Delay1ms(unsigned long volatile msec);
...
int main(void){
  ...
  SysTick_Wait10ms(21);      // d) wait 10ms
   WaitForASLow();    // e) wait for switch to be released
   SysTick_Wait10ms(25);     // f) wait 250ms
   SetVT();           // g) VT signal goes high
   SysTick_Wait10ms(25);     // h) wait 250ms
...
void PortF_Init(void){ volatile unsigned long delay;
 ...
 // GPIO_PORTF_CR_R |= 0x1F;           //Alow changes to PF4-0	
